	...
	return (
		<div className="flex-column center-all">
					...
					<Row>
						...
						>
							<span className={cx('lib-introduce-title')}>
								Giới thiệu sản phẩm và giải pháp thư viện điện tử, thư viện số trong
								các trường học
							</span>
							<span className={cx('lib-introduce-content')}>
								V-Lib là giải pháp phần mềm Thư viện điện tử – Thư viện số được
								Quảng Ích xây dựng và phát triển từ năm 2003. Với tính năng ưu việt
								vượt trội trong quản lý tích hợp nghiệp vụ thư viện truyền thống và
								thư viện điện tử hiện đại, V-Lib đã được ứng dụng thành công tại
								hàng nghìn tổ chức, đơn vị, trường học trên toàn quốc.
							</span>
							<span className={cx('lib-introduce-content')}>
								V-Lib được xây dựng trên nền tảng công nghệ điện toán đám mây, người
								dùng không cần phải cài đặt, chỉ cần có mạng Internet là có thể sử
								dụng được hệ thống ở bất cứ nơi đâu. Đây là sản phẩm được thiết kế
								đáp ứng đầy đủ và tốt nhất cho nhu cầu của nhà quản lý trong việc
								quản lý thư viện. Các chức năng của Hệ thống được thiết kế theo
								chuẩn thống nhất, đáp ứng đầy đủ nghiệp vụ quản lý thư viện trong
								các nhà trường. Đặc biệt hệ thống thư viện cung cấp đầy đủ các mẫu
								biểu báo cáo, phục vụ đắc lực cho ban giám hiệu nhà trường trong quá
								trình theo dõi và quản lý thư viện.
							</span>
						</Col>
					</Row>
				</Container>
			...
			<div
				...
				style={{ width: '100%', height: '864px', backgroundColor: '#EEEEEE' }}
			>
				<section
					...
					<Row className="d-flex ">
						...
						<Col data-aos="fade-left" className="d-flex flex-column">
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>01</span>Là hệ thống tích
								hợp hoàn chỉnh mọi nghiệp vụ hoạt động thư viện truyền thống đã được
								chuẩn hóa theo quy định của Bộ GD&ĐT, Quản lý sách, biên mục theo
								chuẩn thư viện Phổ thông.{' '}
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>02</span>Đạt được hiệu quả
								sử dụng tối ưu nhất cho mọi quy trình nghiệp vụ thư viện, tiết kiệm
								tối đa nguồn lực tài chính, thời gian và công sức…
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>03</span>Khả năng tùy biến
								cao, cho phép cập nhật, thay đổi hoặc nâng cấp nội dung, chức năng
								cũng như công nghệ cho hệ thống.Khả năng tùy biến cao, cho phép cập
								nhật, thay đổi hoặc nâng cấp nội dung, chức năng cũng như công nghệ
								cho hệ thống.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>04</span>Số hóa đầu sách qua
								thiết bị mã vạch hiện đại, số hóa thẻ thư viện, quản lý phân quyền
								chặt chẽ.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>05</span>Chuyển giao và hỗ
								trợ kỹ thuật, hỗ trợ người dùng tốt.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>06</span>Quản lý chi tiết
								hóa đơn của đầu sách nhập về.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>07</span>Thiết lập mã vạch
								tự động cho sách và các tài liệu khác trong thư viện phù hợp với kho
								chứa sách. Đồng thời in mã vạch để dán vào sách hay tài liệu.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>08</span>Mượn trả sách tự
								động qua mã vạch
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>09</span>Cập nhật biên mục
								của từng cuốn sách hay tài liệu.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>10</span>Thống kê tra cứu đa
								dạng, chi tiết và trực quan phục vụ mọi nhóm đối tượng.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>11</span>Có khả năng
								nhập/xuất dữ liệu.
							</span>
							<span className={cx('lib-list')}>
								<span className={cx('lib-list-number')}>12</span>Hệ thống báo cáo đa
								chức năng, đáp ứng mọi nghiệp vụ báo cáo thư viện.
							</span>
						</Col>
					</Row>
				</Container>
			...
			<section
				className="center-all"
				style={{ width: '100%', height: '617px', backgroundColor: '#EEEEEE' }}
			>
				<div
					className="center-all flex-column"
					style={{ height: '100%', width: '1280px', gap: '56px' }}
				>
